...
public class TimedAlarmService {
    public static void trigger(Alarm alarm) {
      if (allTimeCriteriasAreSatisfiedBy(alarm)) {
        AlarmService.triggerAlarm(alarm.nested1);
      }
    }

  private static boolean allTimeCriteriasAreSatisfiedBy(Alarm alarm) {
    return Arrays.stream(alarm.timeCriterias).allMatch(TimedAlarmService::isSatisfied);
  }
