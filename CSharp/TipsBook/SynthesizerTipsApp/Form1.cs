...
public partial class Form1 : Form
{
  ...
  private void ExtractParagraphsFromPdf()
  {
    ...
    try
    {
      using (var reader = new PdfReader(_pdfFilePath))
      using (var pdfDoc = new PdfDocument(reader))
      {
        ...
        foreach (Match match in matches)
        {
          if (match.Groups.Count >= 3)
          {
            ...
            {
              _paragraphs.Add(formattedTip);
            }
          }
        }
                    
        // If we still don't have all tips, try a more aggressive approach
        if (_paragraphs.Count < 10000) // If we're missing many tips
        {
          ...
          foreach (var line in lines)
          {
            ...
            if (match.Success)
            {
              ...
              if (!string.IsNullOrWhiteSpace(tipText) && !_paragraphs.Any(p => p.StartsWith($"{tipNumber}. ")))
              {
                _paragraphs.Add($"{tipNumber}. {tipText}");
              }
            }
          }
        }
  ...
  }        private void ShowRandomTip()
  {
    ...
    var tipIndex = _random.Next(_paragraphs.Count);
    var tipText = _paragraphs[tipIndex];

    // Create a custom tip form and show it
    using var tipForm = new TipForm(tipText, tipIndex + 1, _paragraphs.Count);
